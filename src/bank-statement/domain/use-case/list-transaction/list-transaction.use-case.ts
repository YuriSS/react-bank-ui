...
export class ListTransactionUsecase
  implements Usecase<{}, Promise<Transaction[]>>
{
  public constructor(
  ...
  public async execute(
    ...
    return requestTransactionOutput.transactions.map(
      (rawTransaction: RawTransaction): Transaction =>
        this.createTransactionUsecase.execute(rawTransaction)
    );
  }
}
...
export class ListTransactionUsecaseFactory {
  public static create(
    request: Request,
    api: TransactionApi,
    createTransactionUsecase: Usecase<RawTransaction, Transaction>
  ) {
